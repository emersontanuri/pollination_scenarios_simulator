...
def home():
  ...
  st.write('Também é possível selecionar a opção de visualização em camadas ao invés de empilhado, para quando se deseja verificar a sincronia de florescimento entre os grupos de plantas.')
  ...
  expander.write('   - Média de dias para florescimento')
  expander.write('   - Desvio padrão de dias para florescimento')
  ...
  st.write('2. Para cada flor, o dia de florescimento é calculado usando uma distribuição normal com a média e o desvio padrão fornecidos.')
  st.write('3. Geramos uma lista de dias de florescimento e contamos o número de flores em cada dia.')
  
  st.markdown('### O que é exibido na simulação?')
  st.write('A simulação exibe um gráfico de barras empilhado com o número de flores em cada dia de florescimento para cada grupo de plantas adicionado.')
  st.image('./assets/visualization.png')
  st.write('O eixo x representa a data de florescimento e o eixo y representa o número de flores.')
